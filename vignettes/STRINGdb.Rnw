...
\author{Andrea Franceschini and Damian Szklarczyk}
\date{}
\maketitle
...
Besides, if you specify a local directory to the parameter input-directory, the database files will be downloaded into this directory and most of the methods can be used off-line. Otherwise, the database files will be saved and cached in a temporary directory that will be cleaned automatically when the R session is closed. By setting \code{link_data="detailed"} we also load the evidence score columns, such as the experimental score, in addition to the combined score.\\\\
...
Finally, we extract the genes with p-value < 0.05 and, for the examples below, we focus on the top 200 genes sorted by p-value.
...
If you want more control over the appearance of the network image, the \code{get\_png} and \code{get\_link} methods expose additional styling options. In particular, you can switch between \code{evidence} and \code{confidence} edge styles, hide node labels, hide disconnected proteins, disable the structure pictures inside bubbles, use the flat node design, center node labels, and change the label font size.
...
If you already have a \code{STRINGdb} object, calling \code{set\_background} is sufficient and you do not need to instantiate a new object again. Passing \code{backgroundV} to the constructor is simply an alternative when you already know the background at initialization time:
...
If you just want to know which terms are assigned to your set of proteins, and not necessarily which ones are enriched, you can use "get\_annotations" method. This method outputs the terms from most categories (the exceptions are KEGG terms due to licensing issues and PubMed due to the size of the output) that are associated with your set of proteins. 
...
The protein table also includes the preferred name, the size and a short description of each protein.
...
The "get\_interactions" method returns a data frame describing the interactions among the queried proteins. The table contains the STRING identifiers in the \code{from} and \code{to} columns, the corresponding preferred names in \code{from\_name} and \code{to\_name}, the \code{combined\_score}, and any additional evidence scores available for the current \code{link\_data} setting. This makes it easy to inspect both the interacting proteins and the supporting scores for each interaction.
...
<<interaction_partners_experimental>>=
...
STRING also stores best hits (as measured by bitscore) between proteins from different species. "get\_homologs\_besthits" lets you retrieve these homologs.
