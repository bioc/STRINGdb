...
STRINGdb <- setRefClass("STRINGdb",
  fields = list(
    ...
    proteins = "data.frame",
    aliases_raw = "data.frame",
    aliases_all = "data.frame",
    aliases_tf = "data.frame",
    aliases_tf_unranked = "data.frame",
...
"
    ...
    get_aliases = function(takeFirst = TRUE, usePreferredSources = TRUE) {
      "
Description:
...
"
      ...
      if (!takeFirst & nrow(aliases_all) > 0) {
        return(aliases_all)
      }
      if (takeFirst & usePreferredSources & nrow(aliases_tf) > 0) {
        return(aliases_tf)
      }
      if (takeFirst & !usePreferredSources & nrow(aliases_tf_unranked) > 0) {
        return(aliases_tf_unranked)
      }

      if (nrow(aliases_raw) == 0) {
        ...
        pr1 <- data.frame(STRING_id = proteins$protein_external_id, alias = proteins$preferred_name, sources = "preferred_name", stringsAsFactors = FALSE)
        pr2 <- data.frame(STRING_id = proteins$protein_external_id, alias = proteins$protein_external_id, sources = "STRING_id", stringsAsFactors = FALSE)
        # pr3=data.frame(STRING_id=proteins$protein_external_id, alias=unlist(strsplit(proteins$protein_external_id, "\\."))[seq(from=2, to=2*nrow(proteins), by=2)], sources="protein_short_id", stringsAsFactors=FALSE)

        aliases_raw <<- rbind(pr1, pr2, aliasDf)
      }

      if (nrow(aliases_all) == 0) {
        aliases_all <<- subset(aliases_raw, select = c("STRING_id", "alias"))
      }

      if (takeFirst) {
        if (usePreferredSources) {
          aliases_tf <<- prioritize_aliases_by_source(aliases_raw)
          return(aliases_tf)
        } else {
          proteins <<- get_proteins()
          aliasDf_take_first <- subset(aliases_raw, !(sources %in% c("preferred_name", "STRING_id")) &
            !(toupper(iconv(alias, "WINDOWS-1252", "UTF-8")) %in% toupper(proteins$preferred_name)) &
            !(toupper(iconv(alias, "WINDOWS-1252", "UTF-8")) %in% toupper(proteins$protein_external_id)))
          aliases_tf_unranked <<- rbind(
            subset(aliases_raw, sources == "preferred_name", select = c("STRING_id", "alias")),
            subset(aliases_raw, sources == "STRING_id", select = c("STRING_id", "alias")),
            subset(aliasDf_take_first, select = c("STRING_id", "alias"))
          )
          return(aliases_tf_unranked)
        }
      }
...
'
    ...
    map = function(my_data_frame,
                   ...
                   takeFirst = TRUE, removeUnmappedRows = FALSE, quiet = FALSE, usePreferredSources = TRUE) {
      '
Description:
...
Input parameters:
  ...
  "usePreferredSources"           when takeFirst=TRUE, prioritize aliases using the preferred source order.

Author(s):
   Andrea Franceschini
'

      aliasDf2 <- get_aliases(takeFirst = takeFirst, usePreferredSources = usePreferredSources)
...
'
      ...
      cat(
        "Discarded", nrow(interactions_dataframe) - nrow(nonHomologousIntDf), "interactions (",
...
'
    ...
    show = function() {
      ...
      cat(paste(
        ...
        "interactions: ", length(E(get_graph())),
        sep = ""
      ))
    },
